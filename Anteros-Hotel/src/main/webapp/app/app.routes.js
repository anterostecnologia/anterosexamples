...
angular.module('PilukuApp', [ 'ui.router', 'ngRoute', 'ngResource','ngAnimate', 'PilukuApp.controllers', 'oc.lazyLoad', 'chieffancypants.loadingBar' ])
...
.constant('ASSETS', {
    ...
    pageJs: function(pageName,jsName)
    {
        return appHelper.pagesDir + '/' + pageName + '/js/' + jsName +'.js' ;
    },
    pageCss: function(pageName,jsName)
    {
        return appHelper.pagesDir + '/' + pageName + '/css/' + jsName +'.css' ;
    },
    ...
    getController: function(controllerName, fileName) {
        return appHelper.controllersDir + '/' + controllerName + '/' + fileName +'.js';
    },
})
...
.config(
    ...
    function ($stateProvider,   $urlRouterProvider, $$ocLazyLoadProvider, ASSETS) {
        ...
        .state("app", {
            abstract : true,
            url: "/app",
            templateUrl: appHelper.pageView('home','home'),
            resolve: {
                resources: function($ocLazyLoad){
                    return $ocLazyLoad.load([
                        ASSETS.pageJs('home', 'menu-jquery'),
                        ASSETS.pageJs('home', 'jquery.nicescroll.min'),
                         /**** All Directives *** */
                        ...
                        ASSETS.jsDirective('tables', 'tables'),
        ...
        .state("app.home", {
            url: "/home",
            templateUrl: appHelper.pageView('dashboard','index'),
            resolve: {
                resources: function($ocLazyLoad){
                    return $ocLazyLoad.load([
                        ASSETS.pageCss('dashboard','widgets'),

                        ASSETS.core.liveTile,
                        ASSETS.pageJs('dashboard','dashboard'),
                        ASSETS.pageJs('dashboard','chartist.min'),
                        ASSETS.pageJs('dashboard','build/d3.min'),
                        ASSETS.pageJs('dashboard','build/nv.d3.min'),
                        ASSETS.pageJs('dashboard','weather'),
                        // ASSETS.js('dashboard','sparkline'),
                        ASSETS.pageJs('dashboard','bic_calendar'),
                    ]);
                },
            }
        ...
        .state("app.hotel", {
            url: "/hotel",
            templateUrl: appHelper.pageView('hotel','hotel'),
            resolve: {
                resources: function($ocLazyLoad){
                    return $ocLazyLoad.load([
                        appHelper.getController('hotel','hotel-controller'),
                    ]);
                },
            }
        })
